...
id openharness-connector >/dev/null 2>&1 || useradd --system --home-dir "$STATE" --shell /usr/sbin/nologin openharness-connector
...
chown openharness-connector:openharness-connector "$CONF/token"
...
cp "$SRC/connector-linux/systemd/openharness-connector.service" /etc/systemd/system/openharness-connector.service
systemctl daemon-reload
systemctl enable --now openharness-connector
echo "Installed. Status: systemctl status openharness-connector   Logs: journalctl -u openharness-connector -f"
