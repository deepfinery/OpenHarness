...
| `agents`      | `name`, `providerId`, `systemPrompt`  | `description`, `connections`, `knowledgeBaseIds` (also writable notebooks; first is the fallback destination), `workspace`, `experience`, `maxTurns`, `timeoutSeconds`, `tokenBudget`, `contextCompaction` (default `true`; proactive compression with task-memory checkpoints), `pattern`, `patternConfig`, `effort` (`light`/`medium`/`high`/`extra-high`/`max`/`auto`), `enabled`. The studio configures agents inline on workflow cards; this collection remains for API use. |
