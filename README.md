...
- **Open Harness API:** OpenHarness implements the open [Open Harness API](https://github.com/jeffrschneider/OpenHarness) under `/openharness/v1`, so clients written for that spec can drive it. An administrator creates a workspace key (`oh_sk_…`) under **Integrations → API key**. The capability manifest at `/openharness/v1/harnesses/openharness/capabilities` lists what is supported; see [the adapter guide](docs/openharness-api.md). The spec's own conformance suite runs in CI against it ([results](docs/conformance.md)).
...
| `VECTOR_STORE`                                                                     | `weaviate`                    | Vector store for new knowledge bases: `weaviate`, `qdrant`, `opensearch`, `elasticsearch` or `openai`. Existing bases keep theirs.                                                                                            |
| `QDRANT_URL`, `OPENSEARCH_URL`, `ELASTICSEARCH_URL`, `OPENAI_VECTOR_STORES_URL`    | unset                         | Enable the other stores, with their credentials (`QDRANT_API_KEY`, `OPENSEARCH_USERNAME`/`OPENSEARCH_PASSWORD`, `ELASTICSEARCH_API_KEY`, `OPENAI_VECTOR_STORES_API_KEY`). See [docs/vector-stores.md](docs/vector-stores.md). |
